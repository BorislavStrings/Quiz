define(function() {
    ...
    KMeans.prototype.classify = function(point, distance) {
        ...
        for (var i = 0; i < this.centroids.length; i++) {
            var dist = distance(point, this.centroids[i].vector);
            if (dist < min) {
                min = dist;
                index = i;
            }
    ...
    KMeans.prototype.cluster = function(points, k, distance, snapshotPeriod, snapshotCb) {
        ...
        while (movement) {
            ...
            for (var i = 0; i < points.length; i++) {
                assignment[i] = this.classify(points[i].vector, distance);
            }
            ...
            for (var j = 0; j < k; j++) {
                ...
                var newCentroid = new Array(centroid.vector.length);

                for (var g = 0; g < centroid.vector.length; g++) {
                    var sum = 0;
                    for (var i = 0; i < assigned.length; i++) {
                        sum += assigned[i].vector[g];
                    }
                    newCentroid[g] = sum / assigned.length;

                    if (newCentroid[g] != centroid.vector[g]) {
                        movement = true;
                    }
                }

                this.centroids[j] = {vector: newCentroid, user: {}};
                clusters[j] = assigned;
            }
    ...
    module.exports.kmeans = function(vectors, k, type) {
        return (new KMeans()).cluster(vectors, k, type);
    }
    ...
    module.exports.hcluster = hcluster;
